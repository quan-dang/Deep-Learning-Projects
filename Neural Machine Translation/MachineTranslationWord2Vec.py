...
class MachineTranslation:
    def __init__(self):
        # construct a parser to parse parameters
    ...
    # read the source and target language texts    
    def read_input_file(self,path,num_samples=10e13):
        ...
        for line in lines[: min(num_samples, len(lines) - 1)]:
            ...
            target_text = '\t ' + target_text + ' \n'    # mark \t and \n as the start and end of sequence respectively 
    ...
    # build the vocab set of words for both source and target languages
    ...
    # convert the text data to numeric form that can be used by neural translation machine architecture
    def process_input(self, input_texts, target_texts=None, verbose=True):
        # encoder_input_data contains the input source data and it is a 3D array
        # which includes number of records, number of time steps and size of one-hot encoded vector
        encoder_input_data = np.zeros((len(input_texts), self.max_encoder_seq_length), dtype='float32')

        # decoder_input_data contains the input target data
        decoder_input_data = np.zeros((len(input_texts), self.max_decoder_seq_length), dtype='float32')

        # decoder_target_data contains the target labels
        decoder_target_data = np.zeros((len(input_texts), self.max_decoder_seq_length,1), dtype='float32')
    ...
    def train_test_split(self,num_recs,train_size=0.8):
        rec_indices = np.arange(num_recs)
        np.random.shuffle(rec_indices)
        train_count = int(num_recs*train_size)
    ...
    def decode_sequence(self, input_seq, encoder_model, decoder_model):
        ...
        while not stop_condition:
            ...
            # exit when hit max length or find stop character
    ...
    # inference on the holdout dataset and check the quality of translation
    ...
    def main(self):
        if self.mode == 'train':
            ...
            self.model, self.encoder_model, self.decoder_model = self.train(encoder_input_data_train, decoder_input_data_train, decoder_target_data_train)
            in_list, out_list = self.inference(self.model, encoder_input_data_test, self.encoder_model, self.decoder_model, input_text_test)
            ...
            encoder_input_data, _, _, input_texts, _ = self.process_input(input_texts, '', True)
            in_list, out_list  = self.inference(self.model, encoder_input_data_test, self.encoder_model ,self.decoder_model, input_text_test)
            out_df = pd.DataFrame()
            out_df['English text'] = in_list
            out_df['French text'] = out_list
